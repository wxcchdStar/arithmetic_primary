...
public class No733 {
  ...
  public int[][] floodFill(int[][] image, int sr, int sc, int originColor, int newColor) {
    if (sr < 0 || sr >= image.length) return image;
    if (sc < 0 || sc >= image[0].length) return image;
    if (image[sr][sc] == -1 || image[sr][sc] != originColor) return image;
